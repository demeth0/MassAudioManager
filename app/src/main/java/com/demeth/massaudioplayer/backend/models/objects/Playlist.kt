...
class Playlist {
    companion object {
         ...
         * @see #is_last_audio()
         */
        fun isLastAudio(p: Playlist?): Boolean{
            if(p==null) return true
            return p.isLastAudio()
        }
    }
    private var audios: MutableList<Audio>
     ...
     * @see Playlist
     */
    constructor(audios: List<Audio>){
        this.audios = audios.toMutableList()
        order = ArrayList()
     ...
     * @see Playlist
     */
    constructor(vararg audios: Audio) : this(audios.toList())
     ...
     */
    fun prev(): Audio?{
        current = if(current==-1) order.size-1
     ...
     * @param audio The audio that we want to set as the current audio selected in the playlist.
     * @see #set(int)
     */
    fun set(audio: Audio){
     ...
     */
    fun isLastAudio() = current == order.size-1
     ...
     */
    fun random(mode: Boolean){
        this.random=mode
        if(mode){
            var curAudio = 0
            if(current!=-1) curAudio = order.removeAt(current)
            shuffleList()
            if(current!=-1) order.add(0,curAudio)
        }else{
            orderList()
        }
    }
     ...
     */
    fun extend(extension: List<Audio>){
        for(i in extension.indices){ //resize audio order list to match audio
     ...
     */
    fun view(): List<Audio>{
        val audioList = ArrayList<Audio>(audios.size)
        order.forEach{ audioList.add(audios[it]) }
        return audioList
    }
}
